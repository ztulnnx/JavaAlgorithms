...
public class MinPriorityQuene {
	...
	public void minHeapify(int i){
		...
		int largest = i;
		//如果左孩子val比src[i]的val小,则将其下标存储在largest中
		if (l <= heapSize && quene[l] < quene[i]) {
			largest = l;
		}
		//如果右孩子val比src[i]的val小,则将其下标存储在largest中
	...
	public static void main(String[] args) {
		...
		MinPriorityQuene mph = new MinPriorityQuene();

		for (int i = 0; i < s.length; i++) {
			mph.insert(s[i]);
		}
		System.out.println(mph.minimum());			
		System.out.println(mph.extractMin());
		System.out.println(mph.minimum());
		System.out.println(mph.minimum());
		System.out.println(mph.extractMin());
		System.out.println(mph.extractMin());
